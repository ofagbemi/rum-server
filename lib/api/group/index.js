...
const DEFAULT_TASK_GET_LIMIT = 20;
 ...
 */
exports.getCompletedTasks = function getTasks(params) {
  const groupId = util.sanitizeFirebaseRef(params.groupId);
  const limit   = params.limit || DEFAULT_TASK_GET_LIMIT;
  return new Promise((resolve, reject) => {
    firebase.child(`groups/${groupId}`).once('value', (snapshot) => {
