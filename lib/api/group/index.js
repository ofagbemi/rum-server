...
const TASK_UPDATE_WHITELIST = ['title', 'assignedTo'];
    ...
    const waterfallFns = [
      ...
      function moveTask(callback) {
        ...
        taskRef.once('value', (taskSnapshot) => {
          ...
          completedTask.completer = completerId;
...
exports.updateTask = function updateTask(params) {
  return new Promise((resolve, reject) => {
    const groupId = util.sanitizeFirebaseRef(params.groupId);
    const taskId  = util.sanitizeFirebaseRef(params.taskId);

    const update = _.pick(params.update, TASK_UPDATE_WHITELIST);
    firebase.child(`groups/${groupId}/tasks/${taskId}`).update(update, (err) => {
      if (err) return reject(err);
      return resolve(err);
    });
  });
 ...
 *
 * @param {string} params.groupId
 * @param {boolean} [params.getDeviceId=false] - Only set to true if the result
 * will NOT be sent to a client
 * @param {boolean} [params.getAccessToken=false] - Only set to true if the result
 * will NOT be sent to a client
 */
exports.getMembers = function getMembers(params) {
  return new Promise((resolve, reject) => {
    const groupId = util.sanitizeFirebaseRef(params.groupId);
    const getDeviceId = !!params.getDeviceId;
